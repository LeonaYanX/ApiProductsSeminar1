...
//Доработайте контроллер реализовав статичный файл со статистикой работы кэш.
//Сделайте файл доступным по ссылке.
//Перенесите строку подключения для работы с базой данных в конфигурационный файл приложения
namespace Seminar1.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class MemoryChacheProductsController : ControllerBase
    {
        private readonly ProductsContext _dbContext;
        private IMemoryCache _memoryChache;

        public MemoryChacheProductsController(ProductsContext dbContext, IMemoryCache memoryCache)
        { _dbContext = dbContext; _memoryChache = memoryCache; }
        ...
        public ActionResult<string> GetProductsCsvUrl()
        {
            ...
            if (_memoryChache.TryGetValue("books", out var books))
                return Ok(books);
              ...
              else
              {
                using (_dbContext)
                {
                    ...
                    _memoryChache.Set("products", products, TimeSpan.FromMinutes(60));
                    return Ok(products);
            ...
            {
                if (_memoryChache.TryGetValue("groups", out List<ProductGroupModel>? groups))
                return Ok(groups);
                else
                {
                    using (_dbContext)
                    {
                        ...
                        _memoryChache.Set("groups", groups);
                        return Ok(groups);
                    }
                }
            }
